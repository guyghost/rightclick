...
fn workers_status_line(state: &PluginState) -> String {
    if state.intents.is_empty() {
        return format!(
            "No intents | n New | f Refresh intents | / Global search | : Command search | ? Help | specs: {}",
...
mod tests {
    ...
    fn test_status_line_points_to_specs_when_empty() {
        ...
        assert_eq!(
            plugin.status_line(),
            Some(
                "No intents | n New | f Refresh intents | / Global search | : Command search | ? Help | specs: .rightclick/intents"
                    .to_string()
            )
        );
    }
