...
fn search_input_placeholder(scope: SearchScope) -> &'static str {
    match scope {
        ...
        SearchScope::Commands => "Search commands by plugin, action, shortcut, or id...",
    }
}
...
fn empty_query_hint(scope: SearchScope) -> &'static str {
    match scope {
        ...
        SearchScope::Commands => "Search commands by plugin name, action, shortcut, or id",
    }
}
...
fn no_results_message(query: &str, scope: SearchScope, width: u16) -> String {
    ...
    let message = match scope {
        ...
        SearchScope::Commands => {
            format!("No command, plugin, shortcut, or id matches \"{}\"", query)
        }
    };
...
mod tests {
    ...
    fn test_search_input_copy_is_scope_specific() {
        ...
        assert_eq!(
            ...
            "Search commands by plugin, action, shortcut, or id..."
        );
    }
    ...
    fn test_empty_query_hint_is_scope_specific() {
        ...
        assert_eq!(
            ...
            "Search commands by plugin name, action, shortcut, or id"
        );
    }
    ...
    fn test_no_results_message_uses_compact_hint_when_narrow() {
        ...
        assert!(message.contains("No command, plugin, shortcut, or id matches \"deploy\""));
    ...
    fn test_render_search_overlay_command_scope_uses_command_copy() {
        ...
        assert!(content.contains("Search commands by plugin, action, shortcut, or id"));
        assert!(!content.contains("Search files, commands, project items"));
    }
